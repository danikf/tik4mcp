...
`router-init`, `mikrotik-firewall`, `mikrotik-ip`, `mikrotik-mangle-queue`, `mikrotik-home-wifi`, `mikrotik-vpn`, `mikrotik-hardening`,
`mikrotik-monitoring`). Keep the plugin's tool descriptions and the skills' guidance in sync with the
actual tools in `src/`.
